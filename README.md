...
则非线性量测方程的一阶线性化结果为: $$\begin{aligned}
...
1.  初始时刻,地面系下目标状态位置初值:$p=[x, y, z]=[653.42,567.81,590.61]$
    $m$;速度初值:$v=[v_x,v_y,v_z]=[89.93,61.87,84.41]$
    $m/s$;加速度初值:$a=[a_x,a_y,a_z]$ $m/s^2$;
    目标状态初值的误差:位置误差$\Delta p =[500,500,500]$
    ;速度误差$\Delta v = [50, 50, 50]$;加速度$\Delta a = [5,5,5]$
    (用于构造误差协方差矩阵初值).
...
4.  导引头测量帧频为$10\mathbf{HZ}$,共$40\mathbf{s}$;
...
根据\"当前\"统计模型建立的目标运动离散化方程为 $$\begin{aligned}
    X_k = \Phi_{k|k-1}X_{k-1} + U_{k-1}\bar{a_k} + w_k
\end{aligned}$$ 其中,状态转移矩阵分别为 $$\begin{aligned}
    \Phi_{k|k-1}=\left[
        \begin{array}{c c c}
...
对于加速度方差$\sigma$在每个轴上的分量如式[\[sigma_a\]](#sigma_a){reference-type="eqref"
reference="sigma_a"}表示
$$
\begin{aligned}
...
所以,系统的量测方程为式[\[量测方程\]](#量测方程){reference-type="eqref"
reference="量测方程"}:
$$
\begin{aligned}
    z =
...
在MATLAB中基于以上仿真条件进行时长40s的系统仿真,具体结果如图[1](#fig:预测运动轨迹与真实轨迹三维){reference-type="ref"
reference="fig:预测运动轨迹与真实轨迹三维"} -
[4](#fig:加速度误差曲线){reference-type="ref"
reference="fig:加速度误差曲线"}所示.

从图中可以看出,位置的收敛效果较好,在初始误差较大的情况下仅仅2秒后即可快速收敛;速度误差中x轴速度收敛效果较好,但是y轴和z轴尚未完全收敛,其中x轴和y轴误差在 $\pm$ `<!-- -->`{=html}20 $m/s$ 以内,x轴误差则在$\pm$`<!-- -->`{=html}50$m/s$以内;加速度误差中收敛效果较差,其中x轴和y轴误差在 $\pm$ `<!-- -->`{=html}10以内,z轴误差则在 $\pm$ `<!-- -->`{=html}5以内.

可以看出,在给定40s的时间范围内,应用扩展卡尔曼滤波,对系统状态进行最优最优估计,可以对目标的位置进行较好的估计,但是对速度和加速度的估计较差.

![三维预测运动轨迹与真实轨迹](Fig/预测运动轨迹与真实轨迹三维.png)

![三轴位置跟踪曲线](Fig/三轴位置跟踪曲线.png)

![位置误差曲线](Fig/位置误差曲线.png)

![速度误差曲线](Fig/速度误差曲线.png)

![加速度误差曲线](Fig/加速度误差曲线.png)
